...
void cannonBallViz(double targetPosition, int fieldLength, double velocityX,
                   double velocityY, int timeSteps) {
    ...
    // this function handles target in range 0 to fieldLength, map 0 to 0, and
    // fieldLength to w_width
    ...
    for (int i = 0; i <= timeSteps; ++i) {
        ...
        int xPos =
            static_cast<int>(posX(0, velocityX, time) * w_width) / fieldLength;
        trajectory.push_back(new Circle{
            Point{xPos, static_cast<int>(w_height - posY(0, velocityY, time))},
            8});
